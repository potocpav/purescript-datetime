module PureData.Date
  ...
  , lastDayOfMonth
  , adjust
  , module PureData.Date.Component
  ) where

import Prelude

import PureData.Date.Component (Day, Month(..), Weekday(..), Year)
import Data.Enum (class Enum, class BoundedEnum, toEnum, fromEnum, succ, pred)
import Data.Function.Uncurried (Fn3, runFn3, Fn4, runFn4, Fn6, runFn6)
import Data.Int (fromNumber, floor)
import Math as Math
import Data.Maybe (Maybe(..), fromJust, fromMaybe, isNothing)
import PureData.Time.Duration (class Duration, Days(..), Milliseconds(..), toDuration)
...
fromMillis :: Milliseconds -> Date
fromMillis (Milliseconds t) = let
  msPerDay = 86400000.0

  day' = floor (t / msPerDay)

  dayFromYear :: Number -> Number
  dayFromYear y = 365.0 * (y - 1970.0) + Math.floor((y - 1969.0) / 4.0) - Math.floor((y - 1901.0) / 100.0) + Math.floor((y - 1601.0) / 400.0)

  timeFromYear y = msPerDay * dayFromYear y

  -- yearFromTime :: Number -> Number
  -- yearFromTime t = the largest integral Number y (closest to +∞) such that TimeFromYear(y) ≤ t

  unsafeCast :: forall x. BoundedEnum x => Int -> x
  unsafeCast = unsafePartial fromJust <<< toEnum
  in canonicalDate (unsafeCast 2021) (unsafeCast 5) (unsafeCast 5)